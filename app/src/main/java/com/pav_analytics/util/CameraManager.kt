...
import com.pav_analytics.CSVWritter.saveSensorDataToCSV
...
import com.pav_analytics.FileManager.VisualDistress
import com.pav_analytics.FileManager.getMediaStorageDirectory
import com.pav_analytics.FileManager.saveBitmapToExternalStorage
import com.pav_analytics.sensors.startSensors
import com.pav_analytics.sensors.stopSensors
import java.io.File
...
fun takePhoto(
    ...
    fusedLocationClient.lastLocation.addOnSuccessListener { location: Location? ->
        ...
        captureAndSavePhoto(
            ...
            controller,
            gpsLocation,
            capturedFileNameState,
            onVisualDistressSelected
        )
    }
}
        ...
        object : OnImageCapturedCallback() {
            ...
            override fun onCaptureSuccess(image: ImageProxy) {
                ...
                try {
                    ...
                    val currentTimeOfPicture =
                        SimpleDateFormat("yyyyMMdd_HHmmss", Locale.getDefault()).format(Date())
                    val fileName = "Photo_${currentTimeOfPicture}.png"
                    ...
                    if (filePath != null) {
                        ...
                        val pictureFile = PictureFile(
                            fileName = fileName,
                            isFileSent = FileState.NOT_SENT,
                            visualDistress = null,
                            gpsLocation = gpsLocation
                        )
                        MediaFileStorage.addMediaFile(
                            applicationContext,
                            fileName,
                            pictureFile
                        ) //Create new file
...
fun recordVideo(
    ...
    val fileName = "Video_${currentTimeOfVideo}.mp4"
    val file = File(documentDir, fileName)
    val lock = Any()

    synchronized(lock) {
        //review if recording is in progress
        if (isRecordingInProgress) {
            // Stop recording and clear the keep screen on flag
            recording?.stop()
            recording = null
            stopSensors()
                    ...
                    synchronized(lock) {
                        try {
                            if (event.hasError()) {
                                ...
                                val videoFile = VideoFile(file.name, FileState.NOT_SENT, VideoDevice.MOBILE)
...
fun recordVideo(
    ...
    val documentDir = getMediaStorageDirectory(activity)
    ...
    synchronized(lock) {
        ...
        // Start the recording and set the keep screen on flag
        recording = controller.startRecording(
            FileOutputOptions.Builder(file).build(),
            AudioConfig.create(true),
            ...
            when (event) {
                is VideoRecordEvent.Finalize -> {
                    synchronized(lock) {
                        try {
                            if (event.hasError()) {
                                Toast.makeText(activity, "Data Capture failed", Toast.LENGTH_LONG)
                                    .show()
                            } else {
                                val videoFile =
                                    VideoFile(file.name, FileState.NOT_SENT, VideoDevice.MOBILE)
                                MediaFileStorage.addMediaFile(activity, file.name, videoFile)
                                Toast.makeText(activity, "Data Capture succeed", Toast.LENGTH_LONG)
                                    .show()
                            }
                        } catch (e: Exception) {
                            Log.e("Camera", "Error finalizing video", e)
                            ...
                            stopSensors()
                            saveSensorDataToCSV(activity, currentTimeOfVideo)
                            recording = null
                            isRecordingInProgress = false
                            onRecordingStatusChanged()
                            // Clear the keep screen on flag after recording
                            activity.window.clearFlags(WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON)
                        }
                    }
                }
        ...
        // Set the keep screen on flag when recording starts
