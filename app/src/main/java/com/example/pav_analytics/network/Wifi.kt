...
package com.example.pav_analytics.network
...
class Wifi(private val context: Context) {
    ...
    suspend fun getLargerFile(
        endpoint: String, context: Context, file: File? = null, timeoutMs: Long = 3600000L/4 // 1/4 hour
    ): File {
        val destination = file ?: File(getMediaStorageDirectory(context), endpoint.split("/").last())
